...
public class BackPlugin : LazyPlugin
{
    ...
    public override string Author => "Megghy,熙恩改,2409&trae改";
    public override string Description => GetString("允许玩家传送回死亡地点，传回时有无敌时间");
    ...
    private void Back(CommandArgs args)
    {
        ...
        else if (data != Point.Zero)
        {
            ...
            try
            {
                ...
                // 添加GodTime相关代码
                if (!args.Player.GodMode && MainPlugin.godTimePlayer.Exists((TSPlayer x) => x.Index == args.Player.Index))
                {
                    MainPlugin.removeGodTime(args.Player);
                    args.Player.SendErrorMessage("[i:29]回到死亡地点,无敌时间已提前结束!");
                }
